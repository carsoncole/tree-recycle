require "application_system_test_case"

class Admin::ZonesTest < ApplicationSystemTestCase
  setup do
    @admin_zone = admin_zones(:one)
  end

  test "visiting the index" do
    visit admin_zones_url
    assert_selector "h1", text: "Zones"
  end

  test "should create zone" do
    visit admin_zones_url
    click_on "New zone"

    fill_in "Center address", with: @admin_zone.address
    fill_in "Center city", with: @admin_zone.city
    fill_in "Center state", with: @admin_zone.state
    fill_in "Distance", with: @admin_zone.distance
    fill_in "Name", with: @admin_zone.name
    click_on "Create Zone"

    assert_text "Zone was successfully created"
    click_on "Back"
  end

  test "should update Zone" do
    visit admin_zone_url(@admin_zone)
    click_on "Edit this zone", match: :first

    fill_in "Center address", with: @admin_zone.address
    fill_in "Center city", with: @admin_zone.city
    fill_in "Center state", with: @admin_zone.state
    fill_in "Distance", with: @admin_zone.distance
    fill_in "Name", with: @admin_zone.name
    click_on "Update Zone"

    assert_text "Zone was successfully updated"
    click_on "Back"
  end

  test "should destroy Zone" do
    visit admin_zone_url(@admin_zone)
    click_on "Destroy this zone", match: :first

    assert_text "Zone was successfully destroyed"
  end
end
